...
function ChunkToolbar({ chunk }: ChunkToolbarProps) {
    ...
    const copyCheck = useRef(null);

    const [copied, setCopied] = useState<boolean>(false);
    const [copiedTT, setCopiedTT] = useState<boolean>(false);
        ...
        <div className="flex flex-row items-center mb-2">
            {
                chunk && (
                    <>
                            ...
                            <ul tabIndex={0} className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52">
                                {
                                    languages.map((l) => (
                                        <li key={l.value} className={chunk.language}>
                                            <button onClick={onLanguageChange(l.value)}>{l.name}</button>
                                        </li>
                                    ))
                                }
                            </ul>
                        ...
                        <div className={['tooltip-left', 'tooltip-primary', copiedTT ? 'tooltip-open tooltip' : ''].join(' ')} data-tip="Copied!">
                            <button className="btn btn-primary swap swap-rotate" onClick={onCopyClick}>
