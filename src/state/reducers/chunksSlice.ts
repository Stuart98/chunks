...
export const selectChunksByFolderId = createSelector(
    ...
    (entities: EntityState<Chunk>, folderId: string) =>
        Object.values(entities.entities).filter(
            (entity: Chunk | undefined) => entity?.folderId === folderId
        ) || [] // eslint-disable-line max-len
);

export const selectChunkBySlug = createSelector(
    (state: RootState) => state.chunks,
    (entities: Chunk[], slug: string) =>
        Object.values(entities).find((entity: Chunk) => entity.slug === slug) ||
        []
);
