...
const createSelectByProperty = (
    ...
    return (value: string | boolean): Node | null => {
        for (const [key, node] of Object.entries(
            isRootState(state) ? state.chunks.nodes : state.nodes
        )) {
...
export const selectRootNode = (state: RootState | TreeState) =>
    (isRootState(state) ? state.chunks.nodes : state.nodes)['0'] as Folder;
...
export const selectNodeByActive = (
    state: RootState | TreeState
): ((active: boolean) => Node | null) =>
    createSelectByProperty(state, 'active');
export const selectParents =
    (state: RootState): ((node: Node) => Node[]) =>
    (node) =>
        findParents(state, node);
export const selectLastAddedNode = (state: RootState) =>
    state.chunks.lastAddedNode;
